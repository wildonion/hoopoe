    ...
    fn get_vars(&self) -> Self::Context {
        ...
        let ctx = Context::<Env>{ // context contains a generic of type Env vars
            vars: Env{
                ...
                KAFKA_PORT: std::env::var("KAFKA_PORT").expect("KAFKA_PORT must be there!"),
                KAFKA_HOST: std::env::var("KAFKA_HOST").expect("KAFKA_HOST must be there!"),
                KAFKA_USERNAME: std::env::var("KAFKA_USERNAME").expect("KAFKA_USERNAME must be there!"),
                KAFKA_PASSWORD: std::env::var("KAFKA_PASSWORD").expect("KAFKA_PASSWORD must be there!"),
                ...
                POSTGRES_USER: std::env::var("POSTGRES_USER").expect("POSTGRES_USER must be there!"),
