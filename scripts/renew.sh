...
if [[ $is_new_domain == "Y" || $is_new_domain == "y" ]]; then
    ...
    make sure that you have a (sub)domain points to this machine and it can accepts inbound connections 
    from the internet as well as necessary ports like 80 and 443 are opened"
    NGINX_CONTAINER_ID=$(docker container ls  | grep 'nginx' | awk '{print $1}')
    sudo docker stop $NGINX_CONTAINER_ID # stop nginx docker cause certbox needs the port 80 and 443 to check the connection
    sudo certbot certonly --standalone -d $DOMAIN
    sudo docker start $NGINX_CONTAINER_ID
